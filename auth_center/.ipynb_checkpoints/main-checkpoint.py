...
async def mfa_verify_page(
    ...
    print(f"[mfa/verify] session row: {dict(row) if row else None}")
    ...
    if not (mfa_required == 1 and mfa_verified != 1):
        print(f"[mfa/verify] 非 pending 状态 mfa_required={mfa_required} mfa_verified={mfa_verified}，跳首页")
...
async def startup_event():
    ...
    try:
        ...
        print(f"默认管理员: admin@sso.local / admin123456")
        print("=" * 60)
        print("新增功能:")
        print(" - 手机验证码登录")
        print(" - 多因素认证（MFA）")
