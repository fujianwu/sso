from fastapi import APIRouter, HTTPException, Depends, Response, Request, Form, Header
...
# 在 oauth.py 的 authorize_page 函数中修改
...
async def authorize_page(
    ...
    scope: str = "openid profile email",  # 默认包含基础字段
    ...
    context = {
        ...
        "base_scopes": base_scopes_to_show,  # 基础必选 scope
        "extra_scopes": extra_scopes_to_show,  # 额外可选 scope
        "oauth_fields": available_oauth_fields,  # 所有可选的自定义字段
        "requested_field_keys": [s.split(":", 1)[1] for s in scopes if s.startswith("field:")]  # 请求的字段
    }
...
async def authorize_confirm(
    ...
    return RedirectResponse(
        url=f"{redirect_uri}?{urlencode({k: v for k, v in success_params.items() if v})}",
        status_code=302
    )
...
def build_error_redirect(redirect_uri: str, error: str, error_description: str, state: Optional[str] = None) -> str:
    ...
    return f"{redirect_uri}?{urlencode({k: v for k, v in params.items() if v})}"
...
async def get_userinfo(authorization: Optional[str] = Header(None)):
    ...
    if not user:
        raise HTTPException(
            status_code=404,
            detail={"error": "user_not_found", "error_description": "用户不存在"}
        )
