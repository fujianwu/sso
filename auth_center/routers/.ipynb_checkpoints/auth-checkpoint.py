...
from auth_center.database import (
    UserDB, SessionDB, LoginLogDB, get_db, TrustedDeviceDB
)
...
async def request_password_reset(email: str = Form(...)):
    ...
    # TODO: 实现邮件发送后移除下面的 reset_token 字段
    import os
    resp = {"success": True, "message": "如果该邮箱已注册，您将收到重置链接"}
    if os.getenv("DEBUG", "false").lower() == "true":
        resp["reset_token"] = reset_token  # 仅开发模式暴露
    return resp
